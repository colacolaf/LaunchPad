...
- [x] Disk journal: append-only event log of every engine transition. — **Done 2026-09-17 (decision row 63):** the in-memory command journal (rows 59–60) plus the framed `LPDJ0001` byte format — magic + header riding in the file (so a loaded journal replays without its original engine) + length-prefixed entries, all-u64 LE, zero new deps. Decode is **total**: bad magic, truncation, unknown tags, invalid values, and trailing garbage are `JournalError::Corrupt` — never a panic, never silence. Round-trip file → replay digest equality proven incl. a proptest.
