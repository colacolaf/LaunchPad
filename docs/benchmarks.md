...
**Session #3 (2026-09-16) — Option A: slot arena + intrusive chains. NEGATIVE RESULT, REVERTED.** The full rewrite (arena + O(1) unlink/relink + slot-storing locators, kill the queue descent, the double traversal, and the level alloc/free churn) landed with **100/100 tests unmodified green** and digest/counters byte-identical — and **p50 250→250 ns** (clean run: p90 334→333, p99 667→667). The pre-set ≥5% bar fired; reverted. Conclusion the three experiments compose into: **p50 250 ns is insensitive to the book's internal storage** — at this op size the ensemble (timer ~27% by profile, risk/lock checks, index hash, generator bookkeeping) dominates, and out-of-order execution hides isolated tree-op latency that micro-benchmarks measure truthfully but the stream doesn't pay additively. Scale-up honesty: per-op micro-costs become decisive when ops grow 10× (Phase 3 fees + journaling, Phase 4 networking) — the arena implementation sketch lives in the session record for that day, deliberately not as code debt now. **The plateau rule has met its condition: sessions #1 and #3, two structural experiments, zero p50 movement. Baseline v1.1 stands as Phase 2's honest number.**
