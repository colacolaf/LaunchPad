...
- **Tests:** 92/92 green (85 lib + 7 integration) across both sessions, all four gates (fmt, clippy `-D warnings`, test, release smoke). The three Phase 0 review flags are each closed by code + test: (a) bid-move lock top-up, (b) `KilledFok`, (c) the widened property strategy.
...
- **Next week:** the remaining Phase 1 gate items — edge-case sweep (id reuse after death, cancel-vs-killed-id), the exchange-core surface re-read, then the whole-phase review + fluency gate before closing the phase.
